...
class DatabaseService:
    ...
    def save_heating_data(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            circulation_pump = bool(int(data.get("circulation_pump", 0))) if "circulation_pump" in data else None
            ...
            heating_data = HeatingData(
                ...
                temp_hotwater_target=float(data.get("temp_hotwater_target")) if "temp_hotwater_target" in data else None,
                temp_heating=float(data.get("temp_heating")) if "temp_heating" in data else None,
                temp_solcollector=float(data.get("temp_solcollector")) if "temp_solcollector" in data else None,
                temp_solstorage=float(data.get("temp_solstorage")) if "temp_solstorage" in data else None,
                solar_production=float(data.get("solar_production")) if "solar_production" in data else None,
            ...
            logger.debug(f"Successfully saved heating data at {datetime.fromtimestamp(data['timestamp'])}")
            ...
            with self.get_session() as session:
                ...
                for row in result:
                    data.append({
                        ...
                        "solar_pump": int(row.solar_pump) if row.solar_pump is not None else None,
                        "circulation_pump": int(row.circulation_pump) if row.circulation_pump is not None else None,
                ...
                if not bdf.empty:
                    ...
                    if "starts" in bdf.columns and not bdf["starts"].isna().all() and bdf["starts"].max() > 0:
            ...
            with self.get_session() as session:
                for _, row in df.iterrows():
                    try:
                        ...
                        solar_pump = bool(int(row.get("solar_pump", 0))) if pd.notna(row.get("solar_pump")) else None
                        circulation_pump = bool(int(row.get("circulation_pump", 0))) if pd.notna(row.get("circulation_pump")) else None
                        dhw_pump = bool(int(row.get("dhw_pump", 0))) if pd.notna(row.get("dhw_pump")) else None
                        
                        data = HeatingData(
                            timestamp=int(row["timestamp"]),
                            datetime=datetime.fromtimestamp(int(row["timestamp"])),
                            active=active,
                            modulation=float(row["modulation"]) if pd.notna(row["modulation"]) else None,
                            ...
                            temp_boiler=float(row["temp_boiler"]) if pd.notna(row["temp_boiler"]) else None,
                            temp_hotwater=float(row["temp_hotwater"]) if pd.notna(row["temp_hotwater"]) else None,
                            temp_hotwater_target=float(row["temp_hotwater_target"]) if pd.notna(row["temp_hotwater_target"]) else None,
                            temp_heating=float(row["temp_heating"]) if pd.notna(row["temp_heating"]) else None,
                            temp_solcollector=float(row["temp_solcollector"]) if pd.notna(row["temp_solcollector"]) else None,
                            temp_solstorage=float(row["temp_solstorage"]) if pd.notna(row["temp_solstorage"]) else None,
                            solar_production=float(row["solar_production"]) if pd.notna(row["solar_production"]) else None,
                            solar_pump=solar_pump,
                            circulation_pump=circulation_pump,
                            dhw_pump=dhw_pump,
                        )
